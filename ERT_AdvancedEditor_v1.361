...
class ERTAPP(wx.Frame):
    ...
    #This section is only for book-keeping; helps define what the "global" variables are
        def variableInfo(): #To see what the 'global' variables are
            ...
            #self.electrodesAllShifted: shifted, sorted list of all electrode xdistances
...
#Functions that run as user interacts with app
    #exists as placeholder when functions are added
    ...
    def onReadIn(self, event):
        ...
        if self.inputDataExt.lower() == '.dat':
        ...
        elif self.inputDataExt.lower() == '.txt':
    ...
    def generateXY(self):
                ...
                if self.startPt != 0:
                    ...
                    for c in self.electrodeCols:
                        for i in enumerate(self.dataframeEDIT.iloc[:,c]):
                            self.dataframeEDIT.iloc[i[0],c] = float(i[1]) - float(self.startPt)
        ...
        if self.inputDataExt =='.XYZ' or self.inputDataExt == '.VTK':
            for i in range(0,len(self.dataframeIN)):
                ...
                self.electxDataIN = np.unique(self.electxDataIN)
        else:
            pass
        ...
        self.electrodesIN = sorted(xDataINList)
        self.electrodesAll = self.electrodesIN
        self.electrodesUnique = np.unique(self.electrodesAll)
        self.electState = []
        for i in self.electrodesAll:
            self.electState.append(bool(i*0+1))


        self.electrodesAllShifted = []
        self.electrodesUniqueUnshifted = []
        if self.dataShifted:
            for e in self.electrodesAll:
                self.electrodesAllShifted.append(e-self.startPt)
            for e in self.electrodesUnique:
                self.electrodesUniqueUnshifted.append(e-self.startPt)
    ...
    def onSliderEdit(self):
        ...
        if self.currentChart == 'Graph':
            ...
            if self.sliderVal in self.electrodesAll:

                self.electrodeToggleBtn.Show()

                toggleState = self.electState[int(self.electrodesAll.index(self.sliderVal))]
    ...
    #Event function that updates "Keep" Column of dataframe based on whether data should be edited out or not
                        ...
                        for c in colNums:
                            if type(c) is list:
                                ...
                                for r in self.dataframeEDIT[colNames[c]]:
                                    index = index + 1
                                    if r < float(self.editDataValues[index][0]) and r > float(self.editDataValues[index][1]):
                                        editList[index] = 0
    ...
    def ONtoggle(self,event):
        ...
        if self.ToggleState == True:
            ...
            eIndex = int(self.electrodesAll.index(self.sliderVal))
    ...
    def getClosestElev(self):
        if len(self.inputTxtTopo.GetValue())>0 and 'Enter Topo Filepath Here' not in self.inputTxtTopo.GetValue():
            if self.topoDF['xDist'].max() > max(self.electrodesUnique) or self.topoDF['xDist'].min() > min(self.electrodesUnique):
                ...
                self.electrodeElevs = [[] for k in range(len(self.electrodesAll))]#blank list

                for x in enumerate(self.electrodesAll):
                    ...
                    if nearestTopoxDist == x[1]:
                        ...
                        mapNum = nearestTopoxDist - self.electrodesAll[elecIndex]
                        ...
                        mapNum = self.electrodesAll[elecIndex] - nearestTopoxDist
        ...
        if self.inputDataExt == '.DAT (LS)':
            ...
            for x in enumerate(self.electrodesAll): #Checks if there's already elevation data??
                found = 0
    ...
    def createExportDF(self):
        ...
        keepElectrodesUnshifted = list(np.array(keepElectrodes) - self.startPt)
        ...
        if self.dataShifted:
            electrodes = list(keepElectrodes)
        else:
            electrodes = list(keepElectrodesUnshifted)
